...
void rsa2048_decrypt(void* source_p, void* source_key_exp, void* source_key_mod, void* source_r_sqr, void* dest, int size_p, int size_m){
  volatile uint64_t arr[RSA_LEN] = {0,0,0,0,0,0,0,0};
  volatile uint64_t* ptrp = arr;
  volatile uint64_t* ptre = source_key_exp;
  volatile uint64_t* ptrm = source_key_mod;
  ...
  //sourcep must be 2048 bits
  //printf("\nPLAINTEXT %lx\n", source_p);
  for(int i=0; i<tot_size; i=i+1) {
    //printf("%lx ", *((uint64_t*)source_p+i));
    *mme_plaintext = *((uint64_t*)source_p+i);
  }
