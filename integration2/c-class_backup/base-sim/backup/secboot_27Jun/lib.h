...
int install_key(uint8_t type_of_key,uint8_t crypto_algo,uint8_t source_index,uint8_t tgt_index,void *pointer_to_certificate)
{
    ...
    printf("INSTALL KEY args %x, %x, %x, %x", type_of_key, crypto_algo, source_index, tgt_index);
    if (type_of_key == 2 || type_of_key==3)
    {
...
int auth_data(uint8_t hash_algo, uint8_t crypto_algo, uint8_t pki_store_index,void *mem_addr_start, int length, void *pointer_to_signature)
{
    int result=0;
    ...
    if ((source_key->key_type == 2 || source_key->key_type == 3) && *sign == 0x03)
    {
        ...
        uint8_t hash[32];
        digest_message(data_pointer,length,hash);
