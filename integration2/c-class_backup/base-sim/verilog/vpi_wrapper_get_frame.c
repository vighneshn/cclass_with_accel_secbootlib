 ...
 *     #include "vpi_wrapper_get_frame.h"
 *     void (*vlog_startup_routines[])() = { get_frame_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_get_frame call=get_frame_calltf size=8 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_get_frame vpiSysFuncSized 8 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
/* sft: $imported_get_frame vpiSysFuncSized 8 unsigned */

/* tab: $imported_get_frame call=get_frame_calltf size=8 acc=rw:%TASK */

PLI_INT32 get_frame_sizetf(PLI_BYTE8 *user_data)
{
  return 8u;
}
...
void get_frame_vpi_register()
{
  ...
  tf_data.tfname = "$imported_get_frame";
  tf_data.calltf = get_frame_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = get_frame_sizetf;
  tf_data.user_data = "$imported_get_frame";
