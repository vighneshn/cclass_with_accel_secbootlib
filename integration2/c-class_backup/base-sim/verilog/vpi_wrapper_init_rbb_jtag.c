 ...
 *     #include "vpi_wrapper_init_rbb_jtag.h"
 *     void (*vlog_startup_routines[])() = { init_rbb_jtag_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_init_rbb_jtag call=init_rbb_jtag_calltf size=32 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_init_rbb_jtag vpiSysFuncSized 32 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned int init_rbb_jtag(char );
...
PLI_INT32 init_rbb_jtag_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = init_rbb_jtag(arg_1);
...
/* sft: $imported_init_rbb_jtag vpiSysFuncSized 32 unsigned */

/* tab: $imported_init_rbb_jtag call=init_rbb_jtag_calltf size=32 acc=rw:%TASK */

PLI_INT32 init_rbb_jtag_sizetf(PLI_BYTE8 *user_data)
{
  return 32u;
}
...
void init_rbb_jtag_vpi_register()
{
  ...
  tf_data.tfname = "$imported_init_rbb_jtag";
  tf_data.calltf = init_rbb_jtag_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = init_rbb_jtag_sizetf;
  tf_data.user_data = "$imported_init_rbb_jtag";
